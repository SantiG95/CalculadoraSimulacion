import scipy.stats as stats # Necesario para el ejercicio 12 (Black-Scholes)

# =====================================================================
# 1. MUESTREO POR RECHAZO (Para Ejercicios 1 y 10)
# =====================================================================

def montecarlo_rechazo_pi(n=10000, hacerGrafico=True):
    ...
    print(f"PI Estimado: {pi_estimado:.6f} (Real: {np.pi:.6f})")
    ...
    if hacerGrafico:
        ...
        plt.title(f"Monte Carlo Pi: {pi_estimado:.4f}")
        plt.legend()
        plt.show()
    return pi_estimado

def montecarlo_rechazo_curvas(f_techo, f_piso, a, b, y_min, y_max, n=10000, hacerGrafico=True):
...
def montecarlo_error_maximo(f, a, b, confianza, error_max, n_piloto=1000):
    """
    ...
    print(f"Para garantizar un error < {error_max} al {confianza*100}%, se necesitan N = {n_necesario} puntos.")
...
def simulador_orbital(dv_nominal, sigma_dv, t_nominal, sigma_t, n=100000):
    """
    Resuelve Ej 11: Simula éxito de inserción orbital.
    (Requiere que definas los parámetros de tu problema específico)
    """
    ...
    # (Ejemplo: el impulso total dv*t debe ser mayor a un umbral X)
    impulso_total = dv_real * t_real
    umbral_exito = (dv_nominal * t_nominal) * 0.95 # Ejemplo 95% del nominal
    
    exitos = impulso_total >= umbral_exito
    probabilidad = np.mean(exitos)
    
    print(f"\n--- Ejercicio 11: Simulador Orbital ---")
    print(f"Probabilidad de éxito estimada: {probabilidad*100:.2f}%")
    ...
    print(f"Margen adicional de Delta-V requerido para 99% de éxito: {margen:.2f}")

def simulador_opcion_europea(S0, K, T, r, sigma, n=100000):
    """
    Resuelve Ej 12: Precio de Call Europea (Monte Carlo vs Black-Scholes) y VaR.
    S0: Precio actual, K: Strike, T: Tiempo (años), r: Tasa libre riesgo, sigma: Volatilidad
    """
    # 1. Monte Carlo
    ...
    print(f"Precio Call (Monte Carlo): ${precio_mc:.4f}")
    print(f"Precio Call (Black-Scholes): ${precio_bs:.4f}")
    print(f"Diferencia de validación: ${abs(precio_mc - precio_bs):.6f}")
    print(f"VaR 99% a 1 día de la acción: ${var_99:.4f}")