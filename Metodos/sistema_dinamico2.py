...
tabla_matriz = pd.DataFrame(A, columns=["x", "y"], index=["x'", "y'"])
...
eq1 = sp.Eq(sp.diff(xs(t), t), a*xs(t) + b*ys(t) + e)
eq2 = sp.Eq(sp.diff(ys(t), t), c*xs(t) + d*ys(t) + f)
...
x0_auto = np.linspace(x_min, x_max, int(np.sqrt(cantidad_trayectorias)))
y0_auto = np.linspace(y_min, y_max, int(np.sqrt(cantidad_trayectorias)))
 
for xi in x0_auto:
    for yi in y0_auto:
        _, X_auto = rk4_sistema(np.array([xi, yi]), t0, t_fin, h)
        plt.plot(X_auto[:, 0], X_auto[:, 1], linewidth=1, alpha=0.7)
 
# Trayectoria principal
plt.plot(X_vals[:, 0], X_vals[:, 1], color="black", linewidth=2, label="Trayectoria principal")
plt.scatter(x0, y0, s=80, label="Condicion inicial")
 
# Equilibrio
if equilibrio is not None:
    plt.scatter(equilibrio[0], equilibrio[1], s=120, marker="x", label="Equilibrio")
...
if len(nul_x_sol) > 0:
    y_nul_x = sp.lambdify(x_sym, nul_x_sol[0], "numpy")(x_plot)
    plt.plot(x_plot, y_nul_x, "--", label="Nulclina dx/dt=0")
 
if len(nul_y_sol) > 0:
    y_nul_y = sp.lambdify(x_sym, nul_y_sol[0], "numpy")(x_plot)
    plt.plot(x_plot, y_nul_y, "--", label="Nulclina dy/dt=0")
